...
SELECT 
    gen_random_uuid(),
    '00000000-0000-0000-0000-000000000000', -- プレースホルダー
    'template',
    achievement_name,
    ...
    metadata
FROM (VALUES
    ('first_practice', '初回練習完了', '最初の練習を完了しました', '🎯', '#22c55e', 10, '{"template": true}'),
    ('perfect_score', '初回満点', '初めて満点を獲得しました', '⭐', '#f59e0b', 50, '{"template": true}'),
    ('weekly_streak', '7日連続練習', '7日間連続で練習しました', '🔥', '#ef4444', 100, '{"template": true}'),
    ('practice_master', '100回練習達成', '100回の練習を完了しました', '🏆', '#8b5cf6', 500, '{"template": true}'),
    ('score_improver', 'スコア向上', '平均スコアが1ポイント向上しました', '📈', '#3b82f6', 25, '{"template": true}')
) as t(achievement_name, achievement_description, badge_icon, badge_color, points_earned, metadata)
ON CONFLICT DO NOTHING;

-- テンプレート成果を削除（実際のユーザーIDが設定されていないため）
